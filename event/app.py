...
import os as _os
# Migration artık Railway PRE-DEPLOY adımında (migrate.py) çalışır — web process'inden
# tamamen ayrı. Web process init YAPMAZ → anında ayağa kalkar, GIL'i tutan ağır init işi
# olmadığı için healthcheck düşmez/crash-loop olmaz.
# Railway'de OTOMATİK atlanır (RAILWAY_* env var'ları) — elle SKIP_INIT_DB ayarlamaya gerek
# yok. Yerelde (Railway dışı) inline init çalışır — dev kolaylığı.
_on_railway = bool(
    _os.environ.get("RAILWAY_ENVIRONMENT") or _os.environ.get("RAILWAY_SERVICE_ID")
    or _os.environ.get("RAILWAY_PROJECT_ID")
)
if _os.environ.get("SKIP_INIT_DB") == "1" or _on_railway:
