...
from auth import get_current_user, check_permission as has_permission
...
from models import (
    ...
    TR_CITIES, SUPPLIER_TYPES, Service, SERVICE_CATEGORIES, Request as ReqModel, RequestModule,
    SystemSetting, User, Venue,
...
async def requests_detail(
    ...
    # SystemSetting'den firma bilgileri (imza vb.)
    def _ss(key: str) -> str:
        r = db.query(SystemSetting).filter(SystemSetting.key == key).first()
        return r.value if r else ""
    settings_ctx = {
        "company_name":    _ss("company_name"),
        "company_email":   _ss("company_email"),
        "company_phone":   _ss("company_phone"),
        "email_signature": _ss("email_signature"),
    }
...
async def requests_preview(
    ...
    customer = (db.query(Customer).filter(Customer.id == req.customer_id).first()
                if req.customer_id else None)
    # Settings nesnesini SystemSetting'den oluştur (preview.html uyumluluğu için)
    def _ss(key: str) -> str:
        r = db.query(SystemSetting).filter(SystemSetting.key == key).first()
        return r.value if r else ""
