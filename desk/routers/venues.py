...
async def venues_upload_doc(
    ...
    import uuid as _uuid_mod, storage_helper
    ext = os.path.splitext(doc_file.filename or "")[1].lower() or ".bin"
    safe_name = f"{_uuid_mod.uuid4().hex}{ext}"
    # RBAC v2: company-scoped key
    _cid = getattr(current_user, "company_id", None)
    company_prefix = f"companies/{_cid}/" if _cid else ""
    key = f"{company_prefix}uploads/venue_docs/{venue_id}/{safe_name}"
    _MAX_UPLOAD = 50 * 1024 * 1024
    content = doc_file.file.read(_MAX_UPLOAD + 1)
    ...
    filename = os.path.basename(doc_file.filename or safe_name)
            ...
            try:
                ...
                if key:
                    if storage_helper.R2_ENABLED:
                        storage_helper.delete_file(key)
                    else:
                        safe_path = os.path.abspath(key)
                        upload_dir = os.path.abspath(f"static/uploads/venue_docs/{venue_id}")
                        if safe_path.startswith(upload_dir) and os.path.exists(safe_path):
                            os.remove(safe_path)
            except Exception:
                pass
        else:
