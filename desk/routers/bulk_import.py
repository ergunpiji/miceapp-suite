...
def _validate_venues(rows: list[dict], db: Session) -> list[dict]:
    existing_names = {v.name.lower() for v in db.query(Vendor).all()}
    out = []
    for i, r in enumerate(rows):
        errors = []
        if not r.get("name"):
...
def _save_venues(rows: list[dict], db: Session) -> tuple[int, int]:
    ...
    for r in rows:
        ...
        import json as _j
        cities_json_val = _j.dumps(all_cities, ensure_ascii=False)
        ...
        if existing:
            ...
            existing.cities_json   = cities_json_val
